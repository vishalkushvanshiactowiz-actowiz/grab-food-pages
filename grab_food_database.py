...
from food_data_validation import RestaurantData
...
try:
    ...
    adapter  = TypeAdapter(List[RestaurantData])
    ...
    print("all validate data : ", type(restaurant_detail_list) )
    ...
    try:
        ...
        child_sql = """INSERT IGNORE INTO menu_detail
        ...
        try:
            ...
            for data in restaurant_detail_list:
                rest_details = data.get("restaurant_detail", {})
                parent_values.append((
                    ...
                    rest_details.get("restaurant_image"),
                    rest_details.get("distance"),
                    json.dumps(rest_details.get("opening_time"))  # Dictionary -> JSON String
                ))
            ...
            for data in restaurant_detail_list:
                ...
                menu_items_list = data.get("Menu_Items", [])
                for items_dict_data in menu_items_list:
                    all_child_values.append((
                        items_dict_data.get("restaurant_id"),
                        items_dict_data.get("category_name"),
                        items_dict_data.get("food_id"),
                        items_dict_data.get("food_name"),
                        items_dict_data.get("price"),
                        json.dumps(items_dict_data.get("image_url")),
                        items_dict_data.get("description")
                    ))
            ## call data_commit_batches_wise function for batches wise data save in database.
